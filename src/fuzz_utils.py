...
class TestScenario:
    ...
    def __init__(self, conf):
        ...
        seed_file = os.path.join(conf.seed_dir, conf.cur_scenario)
    ...
    def add_actor(self, actor_type, nav_type, location, rotation, speed, sp_idx, dp_idx):
        """
    ...
    def run_test(self, state, client, tm, world):
        if self.conf.debug:
        ...
        if self.conf.agent_type == c.BASIC or self.conf.agent_type == c.BEHAVIOR:
            if error:
                shutil.copyfile(
